<script lang="ts">
	...
	let playerState: 'menu' | 'camera select' | 'waiting' | 'ready' | 'set' | 'go' | 'clicked' | 'done' =
		$state('camera select');
    ...
    const on: playerFace["on"] = {set:(arg0: number)=> {console.log("we are set, with "+arg0+" to go"); setTimeout(()=>{playerState = 'go'; timer.start()} , arg0)}, event:(e: any)=> {console.log(e)} , again:() => playerState = 'waiting'}
    globalThis.seePlayer = () => {return player};
...
{:else}
	...
	{:else if playerState == 'waiting'}
		<button onclick={()=> {console.log("reading up!") , player.ready(), playerState = 'set'}}> Ready up</button>
